        ...
        <div class="lg:grid lg:grid-cols-[1fr_320px] lg:gap-12">
            ...
            <article>
                ...
                <div class="mb-5 flex flex-wrap items-center gap-x-3 gap-y-1.5">
                    ...
                    @endif
                    <time class="text-xs font-medium text-[var(--color-text-secondary)] opacity-60">
                ...
                {{-- Author bar --}}
                <div class="mb-10 flex items-center justify-between gap-3 border-y border-[var(--color-border)] py-4">
                    {{-- Author info --}}
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex flex-wrap items-baseline gap-x-2 gap-y-0.5 text-xs text-[var(--color-text-secondary)] opacity-70">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button
                            ...
                            class="flex h-9 w-9 items-center justify-center rounded-full border border-[var(--color-border)] text-[var(--color-text-secondary)] opacity-60 transition-all hover:border-[var(--color-accent-secondary)] hover:bg-[var(--color-accent-secondary)] hover:text-[var(--color-accent-primary)] hover:opacity-100"
                        >
                        ...
                        <div
                            ...
                            class="absolute right-0 top-11 z-50 w-48 origin-top-right rounded-xl border border-[var(--color-border)] bg-[var(--color-accent-primary)] p-1.5 shadow-2xl shadow-black/20"
                        >
                            {{-- Twitter / X --}}
                            <a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->url()) }}&text={{ urlencode($post->title) }}"
                               target="_blank" rel="noopener"
                               class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-semibold text-[var(--color-bg-primary)] no-underline transition-colors hover:bg-[var(--color-accent-secondary)] hover:text-[#004225]">
                            ...
                            <div class="my-1 border-t border-[var(--color-bg-primary)] opacity-20"></div>

                            {{-- Copy Link --}}
                            <button
                                @click="navigator.clipboard.writeText('{{ request()->url() }}').then(() => { open = false; })"
                                class="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-semibold text-[var(--color-bg-primary)] transition-colors hover:bg-[var(--color-accent-secondary)] hover:text-[#004225]">
                    ...
                    if($relatedPosts->isNotEmpty()) {
                        ...
                        foreach($relatedPosts->take(2) as $relatedPost) {
                            ...
                            $imgHtml = $img
                                ...
                                : '<div class="m-0 h-[76px] w-[100px] flex-shrink-0 rounded-lg bg-[var(--color-border)] opacity-50"></div>';
                            $title = htmlspecialchars($relatedPost->title);
                            $relatedList .= '<a href="'.$url.'" class="group/bj flex items-center gap-4 no-underline">'.$imgHtml.'<span class="text-sm font-bold leading-snug text-[var(--color-text-primary)] transition-colors group-hover/bj:text-[var(--color-accent-secondary)]">'.$title.'</span></a>';
                        }
                        $bacaJugaHtml = '
                            <div class="not-prose my-10 rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] opacity-40 p-6 font-sans">
                ...
                <div class="prose max-w-none leading-relaxed
                    ...
                    prose-blockquote:border-l-[var(--color-accent-secondary)] prose-blockquote:bg-[var(--color-bg-secondary)] prose-blockquote:opacity-30 prose-blockquote:py-2 prose-blockquote:px-6 prose-blockquote:rounded-r-xl prose-blockquote:not-italic prose-blockquote:text-[var(--color-text-primary)]
                    ...
                    <div class="mt-12 flex flex-wrap gap-2 border-t border-[var(--color-border)] pt-8">
                        @foreach($post->tags as $tag)
                            <a href="{{ route('blog.tag', $tag->slug) }}"
                               class="rounded-full border border-[var(--color-border)] bg-[var(--color-bg-primary)] opacity-50 px-4 py-1.5 text-xs font-medium text-[var(--color-text-secondary)] no-underline transition-all hover:border-[var(--color-accent-secondary)] hover:bg-[var(--color-accent-secondary)] hover:text-[var(--color-accent-primary)] hover:opacity-100">
                                #{{ $tag->name }}
                            </a>
                        @endforeach
                    </div>
                            ...
                            @foreach($latestPosts as $lPost)
                                <article class="group grid gap-6 md:grid-cols-[240px_1fr]">
                                    <a href="{{ route('blog.post.show', [$lPost->category?->slug ?? 'umum', $lPost->slug]) }}" class="relative block aspect-video overflow-hidden rounded-2xl border border-[var(--color-border)] shadow-sm">
                                        ...
                                        @else
                                            <div class="flex h-full w-full items-center justify-center bg-[var(--color-border)] opacity-20 text-[10px] font-bold uppercase tracking-widest text-[var(--color-text-secondary)]">Finlogy</div>
                                        @endif
                                    </a>
                                    <div class="flex flex-col py-1">
                                        <div class="mb-3 flex items-center gap-3">
                                            ...
                                            @endif
                                            <span class="text-[10px] font-bold uppercase tracking-widest text-[var(--color-text-secondary)] opacity-30">{{ optional($lPost->published_at)->translatedFormat('d M Y') }}</span>
                                        </div>
                ...
                <div class="h-full space-y-12">
                    ...
                    <div>
                        <h3 class="mb-6 border-b-2 border-[var(--color-accent-secondary)] pb-2 text-xs font-black uppercase tracking-widest text-[var(--color-text-primary)]">
                            {{ $post->category?->name ?? 'Berita' }} <span class="text-[var(--color-accent-secondary)]">Lainnya</span>
                        </h3>
                            ...
                            @foreach($relatedPosts as $r)
                                <a href="{{ route('blog.post.show', [$r->category?->slug ?? 'umum', $r->slug]) }}"
                                   class="group flex items-center gap-3 py-4 no-underline first:pt-0">
                                    {{-- Thumb --}}
                                    <div class="relative h-[60px] w-[80px] shrink-0 overflow-hidden rounded-lg bg-[var(--color-border)] opacity-50">
                                        ...
                                        @else
                                            <div class="flex h-full w-full items-center justify-center bg-[var(--color-border)] opacity-20 text-[6px] font-bold uppercase tracking-tighter text-[var(--color-text-secondary)]">Finlogy</div>
                                        @endif
                                    </div>
                                    <div class="flex min-w-0 flex-col">
                                        <p class="mb-1.5 line-clamp-2 text-[0.8rem] font-bold leading-tight text-[var(--color-text-primary)] transition-colors group-hover:text-[var(--color-accent-secondary)]">
                                            {{ $r->title }}
                                        </p>
                                        <time class="text-[9px] font-bold uppercase tracking-wider text-[var(--color-text-secondary)] opacity-40">
                    ...
                    <div class="sticky top-24">
                        <h3 class="mb-6 border-b-2 border-[var(--color-accent-secondary)] pb-2 text-xs font-black uppercase tracking-widest text-[var(--color-text-primary)]">
                            Blog <span class="text-[var(--color-accent-secondary)]">Terpopuler</span>
                        </h3>
                        <div class="space-y-4">
                            @foreach($popularPosts as $i => $pop)
                                <a href="{{ route('blog.post.show', [$pop->category?->slug ?? 'umum', $pop->slug]) }}"
                                   class="group flex items-start gap-3 no-underline">
                                    <span class="mt-0.5 shrink-0 text-xl font-black leading-none text-[var(--color-accent-secondary)] opacity-20 transition-colors group-hover:text-[var(--color-accent-secondary)] group-hover:opacity-100">
                                        {{ $i + 1 }}
                                    </span>
                                    {{-- Mini Thumb for Popular --}}
                                    <div class="relative h-[44px] w-[58px] shrink-0 overflow-hidden rounded bg-[var(--color-border)] opacity-40">
                                        ...
                                        @else
                                            <div class="h-full w-full bg-[var(--color-border)] opacity-10"></div>
                                        @endif
                                    </div>
                                    <div>
                                        <p class="line-clamp-2 text-[0.75rem] font-bold leading-tight text-[var(--color-text-primary)] transition-colors group-hover:text-[var(--color-accent-secondary)]">
                                            {{ $pop->title }}
                                        </p>
                                        <div class="mt-1 flex items-center gap-2 text-[8px] font-bold uppercase tracking-widest text-[var(--color-text-secondary)] opacity-30">
