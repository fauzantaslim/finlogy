...
@section('content')
    ...
    @if($posts->isEmpty())
        <div class="rounded-3xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)]/30 py-20 text-center">
            <p class="text-sm font-medium text-[var(--color-text-secondary)]/50 italic">Belum ada artikel dengan tag ini.</p>
        ...
        <div class="space-y-8 divide-y divide-[var(--color-border)]">
            @foreach($posts as $post)
                <article class="group grid gap-6 pt-8 first:pt-0 md:grid-cols-[240px_1fr]">
                    ...
                    @else
                        <div class="aspect-video rounded-2xl bg-gradient-to-br from-[var(--color-accent-primary)]/5 to-[var(--color-border)] flex items-center justify-center">
                    ...
                    <div class="flex flex-col py-1">
                        <div class="mb-2 flex items-center gap-3">
                            @if($post->category)
                                <a href="{{ route('blog.category', $post->category->slug) }}"
                                   class="rounded-sm bg-[var(--color-accent-primary)]/10 px-2 py-0.5 text-[9px] font-black uppercase tracking-widest text-[var(--color-accent-primary)] no-underline">
                                    {{ $post->category->name }}
                                </a>
                            @endif
                            <time class="text-[10px] font-bold uppercase tracking-widest text-[var(--color-text-secondary)]/40">
                        ...
                        <div class="mt-auto flex items-center gap-2 text-xs font-semibold text-[var(--color-text-secondary)]/40">
        ...
        <aside class="mt-16 lg:mt-0">
            <div class="h-full space-y-12">
                ...
                <div class="sticky top-24">
                    ...
                    {{-- Categories Topic Cloud --}}
                    @if(isset($categories) && $categories->isNotEmpty())
                        <div class="mt-12 rounded-3xl bg-[var(--color-bg-secondary)]/50 p-6 border border-[var(--color-border)]/50">
                            <h4 class="mb-4 text-[9px] font-black uppercase tracking-widest text-[var(--color-text-primary)]/50">Topik Utama</h4>
                            <div class="flex flex-wrap gap-2">
                                @foreach($categories as $cat)
                                    <a href="{{ route('blog.category', $cat->slug) }}" class="rounded-full border border-[var(--color-border)] bg-white px-3 py-1.5 text-[10px] font-bold text-[var(--color-text-secondary)] no-underline hover:border-[var(--color-accent-primary)] hover:text-[var(--color-accent-primary)]">
                                        {{ $cat->name }}
                                    </a>
                                @endforeach
                            </div>
